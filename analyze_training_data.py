...
5. Duplicate card analysis (unique to infinite variant)
6. Decision pattern analysis by street/position
7. Agent Tendencies (VPIP, PFR, AF)
8. Logical Sanity Checks
9. Export utilities for ML
...
def print_hand_replay(hand: HandSummary, verbose: bool = True):
    ...
    if verbose:
        ...
        for action in hand.actions:
            ...
            print(f"  P{action['position']} ({pos_name}): {action_str:12} (pot: {action['pot']}, to_call: {action['to_call']})")
...
def analyze_agent_tendencies(data: List[Dict]):
    """Calculate standard poker statistics (VPIP, PFR, AF) across the dataset."""
    print("\n" + "=" * 60)
    ...
    for hid, hand in hands.items():
        ...
        postflop_actions = [a for a in hand.actions if a['street'] in ['flop', 'turn', 'river']]

        # VPIP: Did anyone call or raise preflop? (Excluding big blind forced check)
        if any(a['action'] in ['call', 'raise'] for a in preflop_actions):
            voluntarily_played += 1
        
        # PFR: Did anyone raise preflop?
        if any(a['action'] == 'raise' for a in preflop_actions):
            preflop_raises += 1

        # AF components
        postflop_calls += sum(1 for a in postflop_actions if a['action'] == 'call')
        postflop_raises += sum(1 for a in postflop_actions if a['action'] == 'raise')

    vpip_pct = (voluntarily_played / num_hands) * 100 if num_hands else 0
    pfr_pct = (preflop_raises / num_hands) * 100 if num_hands else 0
    af = (postflop_raises / max(1, postflop_calls))

    print(f"Overall VPIP (Voluntarily Put in Pot): {vpip_pct:.1f}%")
    print(f"Overall PFR (Preflop Raise):           {pfr_pct:.1f}%")
    print(f"Postflop Aggression Factor (AF):       {af:.2f}")


def validate_decision_logic(data: List[Dict], manual_review_count: int = 3):
    """Run programmatic sanity checks and sample hands for manual review."""
    print("\n" + "=" * 60)
    ...
    for d in data:
        # Check 1: Folding when checking is an option (to_call == 0)
        if d['action'] == 'fold' and d['to_call'] == 0:
            illogical_folds += 1
            
        # Check 2: Raising more than 10x the pot (Usually an indicator of a bug in sizing logic)
        if d['action'] == 'raise' and d['raise_amount'] and d['pot_size'] > 0:
            if d['raise_amount'] > (d['pot_size'] * 10):
                massive_overbets += 1
    ...
    if illogical_folds == 0 and massive_overbets == 0:
        ...
        print("  ! WARNING: Found anomalies in decision logic. Review agent code.")

    print(f"\nSampling {manual_review_count} random large-pot hands for manual logical review...")
    hands = reconstruct_hands(data)
    
    # Sort hands by total pot size to find "interesting" ones
    sorted_hands = sorted(
        hands.values(), 
        key=lambda h: max(a['pot'] for a in h.actions) if h.actions else 0, 
        reverse=True
    )
    
    # Grab a few from the top 10% biggest pots to review
    subset = sorted_hands[:max(10, len(sorted_hands)//10)]
    review_sample = random.sample(subset, min(manual_review_count, len(subset)))
    
    for hand in review_sample:
...
def main(filepath: str):
    ...
    if HAS_MATPLOTLIB:
        ...
        plot_action_distribution(data, os.path.join(GRAPHS_DIR, 'action_distribution.png'))
        plot_reward_distribution(data, os.path.join(GRAPHS_DIR, 'reward_distribution.png'))
        plot_pot_sizes(data, os.path.join(GRAPHS_DIR, 'pot_sizes.png'))
